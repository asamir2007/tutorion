...
package ttcc {
	...
	public class Application implements IApplication {
		...
		public function get_consoleVC():VCConsole {
			return consoleVC;
		}
		private var ac:DisplayObjectContainer;
		private static var consoleVC:VCConsole;
		...
		public function get_aer():AER {return aer;}
		public function set_aer(a:AER):void {aer = a;}
		...
		private function prepareConsole(appW:uint, appH:uint, consoleH:uint=400, consoleAlpha:Number=1, consoleBGAlpha:Number=.85):void {
			...
			consoleVC=new VCConsole;
			consoleVC.get_displayObject().alpha=consoleAlpha;
			consoleVC.get_displayObject().visible=false;
			consoleVC.construct(null,c);
			consoleVC.get_displayObject().y=appH-consoleH;	
			ac.addChild(consoleVC.get_displayObject());
			
			
			//keyboard
			k=new Kb(ac);
			k.onKeyDown=function(keyCode:int):void {
				if (keyCode==192) {consoleVC.get_displayObject().visible=!consoleVC.get_displayObject().visible;}
			}
			
		}
		public static function setVisibilityConsole(a:Boolean):void {consoleVC.get_displayObject().visible=a;}
		public static function getVisibilityConsole():Boolean {return consoleVC.get_displayObject().visible;}
