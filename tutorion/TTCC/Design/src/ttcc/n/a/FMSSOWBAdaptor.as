	...
	public class FMSSOWBAdaptor {
		...
		**/
		private function el_so(e:SyncEvent):void {
			...
			switch (l.type) {
				case "clearpage":
					el(this, ID_E_REACT, [ID_M_CLEAR_PAGE, l.p]);
					break;
				case "clearall":
				case "load":
				...
				case "del_item":
					...
					var newObjectsList:Array=[];
					if (!so.data.pages[l.p]) {log(2,'!so.data.pages[l.p]',1);return;}
					var objectsListRaw:Array=so.data.pages[l.p].draw;
					for each(var j:Object in objectsListRaw) {newObjectsList.push(j.uid);}
					el(this, ID_E_REACT, [ID_M_NEW_OBJECTS_LIST, l.p, newObjectsList]);
				...
				case "del_page":
					...
					break;
				case "select_page":
					el(this, ID_E_REACT, [ID_M_SELECTPAGE, l.p, l.user]);
					break;		
				default:
		...
		private function getElementBy(a:Array, pname:String, pval:Object):Object {
			for each(var i:Object in a) {
				if (i[pname]==pval) {return i;}
			}
			return null;
		}

		//{ =*^_^*= id
		public static const ID_M_CLEAR_PAGE:String="ID_M_CLEAR_PAGE";
		/**
		 * wbvp.initAll(so.data.pages,so.data.sel);
		 */
		...
		/**
		 * pagenumber, objectslist
		 */
		public static const ID_M_NEW_OBJECTS_LIST:String="ID_M_NEW_OBJECTS_LIST";
		/**
		 * wbvp.addPage([so.data.pages[l.p]], l.p);
		 */
