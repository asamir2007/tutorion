...
package ttcc.v.mp {
	...
	public class VCMainPanelUsersListElement extends VCMainPanelBasePanelElement implements VCMainPanelIPanelElement {
		...
		private function prepareView():void {
			...
			//com=new JPanel(new FlowLayout(FlowLayout.LEFT,2,0,false));
			cpanel=new JPanel(new FlowLayout(FlowLayout.LEFT,2,0,false));
			com=new JScrollPane(cpanel,JScrollPane.SCROLLBAR_NEVER,JScrollPane.SCROLLBAR_NEVER);
		...
		private function ui_redrawUserList():void {
			...
			var i:MPDUUser;
			var moveToBeginning:Array=[];
			for each(i in mpduUserList) {
				if (i.get_du().get_handIsUp()) {
					moveToBeginning.push(i);
				}
			}
			for each(i in moveToBeginning) {
				mpduUserList.splice(mpduUserList.indexOf(i),1);
			}
			mpduUserList.unshift.apply(mpduUserList, moveToBeginning);
		...
		private function createElement(du:DUUD):VCMainPanelUsersListElementElement {
			var listElement:VCMainPanelUsersListElementElement=new VCMainPanelUsersListElementElement(
				du.get_id()
				,VCMChat.createAvatarSmallPicture(du.get_avatarPictureSmall(),avatarW,avatarH)
				,iconH*3,iconW*3
			);
			...
			listElement.addIcon(
				MPDUUser.ID_ICON_USER_PROPERTY_STREAM
				, new Bitmap(iconStreamOn, PixelSnapping.ALWAYS)
				, new Bitmap(iconStreamOff, PixelSnapping.ALWAYS), iconW, iconH);
			listElement.addIcon(MPDUUser.ID_ICON_USER_PROPERTY_CHAT_TYPING
				, new Bitmap(iconChatTypingOn, PixelSnapping.ALWAYS)
				, new Bitmap(iconChatTypingOff, PixelSnapping.ALWAYS), iconW, iconH);
			listElement.addIcon(MPDUUser.ID_ICON_USER_PROPERTY_HAND
			, new Bitmap(iconHandUpOn, PixelSnapping.ALWAYS)
			, new Bitmap(iconHandUpOff, PixelSnapping.ALWAYS), iconW, iconH);
			return listElement;
		}
...
*/		
		...
		private function createBitmap(w:uint, h:uint, c:uint):Bitmap {
			return new Bitmap(new BitmapData(w,h,false,c), PixelSnapping.ALWAYS);
		}
		//} ^_^ END OF test
		...
		private function processUser(u:DUUD, loggedOut:Boolean=false):void {
			//LOG(0, 'processUser>>'+traceObject(u.toObject()),1);
		...
		private function el_M(targetModel:MPUsersListComModel, elementID:uint, updateFromDataport:Boolean=false):void {
			switch (elementID) {
				...
				//LOG(0,'>>MPUsersListComModel.ID_P_UPDATE_USER_PROPERTIES', 1);
				ui_redrawUserList();
				break;
...
class MPDUUser {
	...
	public function projectDU():void {
		...
		//LOG(3,'projectDU>du:'+traceObject(du.toObject()));
		
		var streamIsOn:Boolean=false;
		if ((du.get_streamName()!=null)&&(du.get_streamName().length>0)&&du.get_streamingIsOn()) {
			streamIsOn=true;
		}
		
		vc.setIconFrame(ID_ICON_USER_PROPERTY_STREAM, uint(streamIsOn));
	}
	...
	public function get_du():DUUD {return du;}
	private var du:DUUD;
