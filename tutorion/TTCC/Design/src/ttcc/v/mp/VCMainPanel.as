package ttcc.v.mp {
	...
	public class VCMainPanel extends AVC {
		...
		public function setReplayMode(a:Boolean):void {
			// block menu
			menu_windowsLayouts.getParent().remove(menu_windowsLayouts);
			for each(var i:VCMainPanelIPanelElement in elementsList) {
				if ((i.getID()!=MACReplay.COMPONENT_ID) && (i.getID()!=MMP.S_ID_B_START)) {
					i.getComponent().mouseChildren=false;
					i.getComponent().setEnabled(false);
				}
			}
		...
		public function setBGColor(color:uint, alpha:Number):void {
			vc.setBackgroundDecorator(new SolidBackground(new ASColor(color, alpha)));
		}
		...
		private function configureVC():void {
			...
			if (panelWidth!=-1) {/*LOG('!=-1, setw',1);*/setW(panelWidth);}
		...
		public function setW(w:int):void {
			//LOG(0, 'setW>>>>>>>>>> w='+w,0);
			if (!vc) {/*LOG(0, 'setW>!vc',1);*/panelWidth=w;return;}
			vc.setWidth((panelWidth==-1)?w:panelWidth);
			if (panelWidth!=-1) {panelWidth=-1;}
			//redraw bg
			vcContainer.graphics.clear();
			vcContainer.graphics.lineStyle(1, 0xAFB2B6);
			vcContainer.graphics.moveTo(0, vc.getHeight());
			vcContainer.graphics.lineTo(w, vc.getHeight());
		}
		...
		private var menu_windowsLayouts:JMenu;
