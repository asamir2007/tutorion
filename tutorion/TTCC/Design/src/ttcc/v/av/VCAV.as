	...
	public class VCAV extends AVCWW {
		...
		private function updateButtons():void {
			...
			var buttonID:Array=[//[[state0, state1]]
				[S_ID_B_SETTINGS, S_ID_B_SETTINGS]
			...
			var buttonID_length:int=buttonID.length;
			for (var j:int = 0; j < buttonID_length;j++) {if (ui_cfg_disabledButtons[j]) {buttonID[j]=null;}}
			for (j= 0; j < buttonID_length;j++) {if (buttonID[j]==null) {
				buttonID.splice(j,1);
				buttonResID.splice(j,1);
				stateID.splice(j,1);
			}}
			...
			state=uint(getStateFor(stateID[i]));
				...
				panelButtons.append(Lib.createIconifiedButton(
					buttonID[i][state]
					,el_b
					,""
					,resO[buttonResID[i][state]]
					,true, true
					,bDisplayNames[buttonResID[i][state]]
				));
			}
		...
		/**
		 * 
		 * @param	buttonID 5 - stream button
		 */
		public function setButtonAvailable(buttonID:int, available:Boolean):void {
			ui_cfg_disabledButtons[buttonID]=!available;
		}
		private var ui_cfg_disabledButtons:Array=[];
		...
		public static const ID_B_SETTINGS:int=nextID++;
		public static const ID_B_TOGGLE_AU_OUT_ON:int=nextID++;
		public static const ID_B_TOGGLE_AU_OUT_OFF:int=nextID++;
		public static const ID_B_TOGGLE_V_OUT_ON:int=nextID++;
		public static const ID_B_TOGGLE_V_OUT_OFF:int=nextID++;
		
		public static const ID_B_TOGGLE_AU_IN_ON:int=nextID++;
		public static const ID_B_TOGGLE_AU_IN_OFF:int=nextID++;
		public static const ID_B_TOGGLE_V_IN_ON:int=nextID++;
		public static const ID_B_TOGGLE_V_IN_OFF:int=nextID++;
		
		public static const ID_B_TOGGLE_S_ON:int=nextID++;
		public static const ID_B_TOGGLE_S_OFF:int=nextID++;
		//} =*^_^*= =*^_^*= END OF id
