	...
	public class MApp extends AM {
		...
		public override function listen(eventType:String, details:Object):void {
			...
			case MAppStartup.ID_E_STARTEDUP:
				// Entry point
				e.subscribe(this, MFMSServerProxy.ID_E_CONNECTION_LOST);
				e.subscribe(this, MServerMonitor.ID_E_CONNECTION_LOST);
				e.subscribe(this, MServerMonitor.ID_E_SUSPEND_APPLICATION);
				e.subscribe(this, MServerMonitor.ID_E_RESUME_APPLICATION);
				
				if (!a.get_ds().get_replayMode()) {
					e.listen(MUserData.ID_A_CHECK_LECTOR_IS_PRESENT, null);
				} else {
					e.listen(VCMReplay.ID_A_SET_VISIBILITY, 1);
				}
				//test
				break;
				
			case MServerMonitor.ID_E_SUSPEND_APPLICATION:
				e.listen(VCMMainScreen.ID_A_DISPLAY_SUSPEND_MESSAGE, true);
				break;
			case MServerMonitor.ID_E_RESUME_APPLICATION:
				e.listen(VCMMainScreen.ID_A_DISPLAY_SUSPEND_MESSAGE, false);
				break;
				
			case MServerMonitor.ID_E_CONNECTION_LOST:
				e.listen(MFMSServerProxy.ID_A_CLOSE_CONNECTION, null);
