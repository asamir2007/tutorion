...
package ttcc.c.ma.ac {
	...
	public class MACAV extends MAPPComponent {
		...
		protected override function prepareBeforeStartup():void {
			...
			var resFileName:Array=[//icons
				['video_settings_w', 'video_settings_b', 'video_settings_p']
				,['cam_off_in', 'cam_off_in_b',  'cam_off_in_p']
				,['cam_on_in', 'cam_on_in_b',  'cam_on_in_p']
				,['cam_off_out', 'cam_off_out_b',  'cam_off_out_p']
				,['cam_on_out', 'cam_on_out_b',  'cam_on_out_p']
				,['sound_off_in', 'sound_off_in_b',  'sound_off_in_p']
				,['sound_on_in', 'sound_on_in_b',  'sound_on_in_p']
				,['sound_off_out', 'sound_off_out_b',  'sound_off_out_p']
				,['sound_on_out', 'sound_on_out_b',  'sound_on_out_p']
				,['stream_off_out', 'stream_off_out_b',  'stream_off_out_p']
				,['stream_on_out', 'stream_on_out_b',  'stream_on_out_p']
			];
			var l:uint = resFileName.length;
			for (var i:int = 0;i < l;i++ ) {
				c.setResourceO(resAppID[i], 
					[
						PictureStoreroom.getPicture(resFileName[i][0])
						, PictureStoreroom.getPicture(resFileName[i][1])
						, PictureStoreroom.getPicture(resFileName[i][2])
					]
				);
		...
		protected override function displayMPButton():void {
			...
			e.listen(MACMP.ID_A_ADD_BUTTON, new DUMPButton(
				...
				,"video_b"
				,"video_p"
				,APP.lText().get_TEXT(Text.ID_TEXT_VIDEO_WINDOW_TITLE)
			));
			
			
			e.listen(MACMP.ID_A_ADD_BUTTON, new DUMPButton(COMPONENT_ID+'stream',400000
				,"stream_off_out"
				,"stream_off_out_b"
				,"stream_off_out_p"
				,'вкл выкл поток'
			...
			e.listen(MACMP.ID_A_ADD_BUTTON, new DUMPButton(COMPONENT_ID+'streamA',400002
				,"sound_off_out"
				,"sound_off_out_b"
				,"sound_off_out_p"
				,'вкл выкл микрофон'
			...
			e.listen(MACMP.ID_A_ADD_BUTTON, new DUMPButton(COMPONENT_ID+'streamV',400001
				,"cam_off_out"
				,"cam_off_out_b"
				,"cam_off_out_p"
				,'вкл выкл камеру'
