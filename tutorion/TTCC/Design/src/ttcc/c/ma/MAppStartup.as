	...
	public class MAppStartup extends AM {
		...
		public override function listen(eventType:String, details:Object):void {
			...
			case MAppData.ID_E_GET_APPLICATION_AND_COMPONENTS_CONFIG_DATA:
				// store duconfig
				r=details;
				var res_:DUAppComponentsCfg=r.get_result();
				APP.setlText(res_.get_uiText());
				a.get_ds().set_duAppComponentsCfg(res_);
				
				// register main screen
				e.listen(VCMMainScreen.ID_A_REGISTER_SINGLETON_VC, a.get_mainScreen());
				
				if (a.get_ds().get_replayMode()&& (a.get_ds().get_startupEnvDataReplay().get_roomID()!=a.get_ds().get_startupEnvData().get_roomID())) {
				...
				listen(M_USERDATA_CONNECTED, DUUD.getLectorSample());
				break;
				
			case ID_A_PREPARE_NETWORK1:
				//log(0,'ID_A_PREPARE_NETWORK1',2)
				var options:Object={swfVersion:2};
			...
			case MFMSServerProxy.ID_E_CONNECTION_ESTABLISHED:
				...
				e.listen(MServerMonitor.ID_A_CONNECT, null);
				if (!a.get_ds().get_replayMode()) {
			...
			case MFMSServerProxy.ID_E_CONNECTION_LOST:
				...
				e.listen(VCMMainScreen.ID_A_DISPLAY_DISCONNECTED_MESSAGE, null);
				break;
			...
			case COMPLETED:
				...
				if (a.get_ds().get_replayMode()) {
					...
					a.get_mainScreen().setReplayMode(true);
				}
				//break;
				//e.listen(VCMMainScreen.ID_A_DISPLAY_NOTIFICATION, {title:'text', text:"aaaaa\nbbb"});
		...
		private function setApplicationBackgroundImage():void {
			a.get_mainScreen().setBgImage(PictureStoreroom.getPicture("bg2"));
		}
