	...
	public class VCMPresentationLoader extends AVCM {
		...
		private function placePages(numPages:uint):void {
			...
			for (var i:uint = 0; i < numPages;i++) {
				page=new BookPage(pagesURL[i], new Bitmap(loaderImBD), new Bitmap(brokenImBD), 210,300);
				bookPages.push(page);
...
class BookPage {
	...
	public function loadImage():void {
		...
		d.addChild(loaderIm);
		imLoader=new Im(imageURL, null, null, el_ImCompleteError, el_ImCompleteError);
	...
	private function el_ImCompleteError(target:Im, errorOccured:Boolean=false):void {
		if (errorOccured) {
