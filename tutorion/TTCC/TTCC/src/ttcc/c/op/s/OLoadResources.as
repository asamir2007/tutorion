...
package ttcc.c.op.s {
	...
	public class OLoadResources extends AO {
		...
		function OLoadResources (onComplete:Function, resourcesPathsList:Array, envData:DUAppEnvData) {
		...
		private function checkLoadedResources(loadingComplete:Boolean):void {
			var criticalGroupIsValid:Boolean;
			var j:DUResource;
			var i:Array;
			...
			aaa: for (var gname:String in groupsList) {
				...
				j=i[0];
				if (!j.get_groupIsCritical()) {continue;}//skip
				bbb: for each(j in i) {
				//log('j.get_groupIsCritical:'+j.get_groupIsCritical());
				//log('j.get_filename:'+j.get_filename());
					if (j.get_groupIsCritical()&&j.get_loaded()) {//atleast one
						//log('atleast one:'+j.get_filename());	
						criticalGroupIsValid=true;
