	...
	public class MFMSServerProxy extends AM {
		...
		public override function listen(eventType:String, details:Object):void {
			...
			case ID_A_SETUP_CONNECTION:
				connectToFMS(details.duenv, details.dupaths, details.timeout);
				break;
		...
		private function connectToFMS(duenv:DUAppEnvData, dupaths:DUApiAndStoragePaths, timeout:int):void {
			if (!ncFMS) {
				ncFMS=new NetConnector;
				ncFMS.constuct(el_ncFMS);
			}
			log(5, 'connecting to fms, arguments:'+[
		...
		//{ =*^_^*= id
		/**
		 * data:{duenv:DUAppEnvData, dupaths:DUApiAndStoragePaths, timeout:int//connection timeout//}
		 */
