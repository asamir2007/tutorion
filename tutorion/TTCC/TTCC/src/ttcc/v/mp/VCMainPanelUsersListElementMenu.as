...
package ttcc.v.mp {
	...
	public class VCMainPanelUsersListElementMenu {
		...
		private function prepareView():void {
			...
			addTextItem(un+((targetUD.get_id()==clientUD.get_id())?' (its you)':""));
			if (targetUD.get_id()==clientUD.get_id()) {
				addButtonItem('handOn', PictureStoreroom.getPicture('ruka_5'), 'поднять руку');
				addButtonItem('handOff', PictureStoreroom.getPicture('ruka_2'), 'опустить руку'); 
			}
...
*/		
		public function addButtonItem(id:String, icon:DisplayObject, hint:String):void {
			//pb.append(createButtonItem(id, icon, hint));
			pb.addChild(createButtonItem(id, icon, hint));
			redraw();
		}
		...
		private function redraw():void {
			...
			vc.graphics.drawRoundRect(0,0,vc.width+10,vc.height+10,3,3);
		}
		
		private function createTextItem(text:String):JLabel {
			var l:JLabel=new JLabel(text+' ');
			l.setBackgroundDecorator(new SolidBackground(new ASColor(0,0)));
			l.pack();
			return l;	
		}
