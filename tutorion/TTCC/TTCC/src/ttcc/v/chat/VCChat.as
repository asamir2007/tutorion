package ttcc.v.chat {
	...
	public class VCChat extends AVCWW {
		...
		public function setResources(windowTitle:String, contactsButtonImage:DisplayObject, enterButtonImage:DisplayObject, windowIconImage:DisplayObject, wndMinW:uint=500, wndMinH:uint=250, inputTextMinH:uint=20,inputTextMaxH:uint=50, sendButtonWidth:Number=-1):void {
		...
		*/
		...
		private function configureVC():void {
			...
			it.addEventListener(KeyboardEvent.KEY_DOWN, function(e:KeyboardEvent):void {
				if (KeyboardEvent(e).keyCode==Keyboard.ENTER) {listener(chatRef, ID_E_BUTTON_SEND);}
			});
			...
			if (b_width!=-1) {b.setMaximumWidth(b_width);}
